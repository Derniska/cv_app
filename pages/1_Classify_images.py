...
with st.spinner('Please wait, application is initializing...'):
    ...
    DB_PATH = '/home/jovyan/dlba/dlba_course_miba_25/topic_09/app/data/db'
...
if uploaded_file is not None:
    ...
    if '.jpg' in file_name:
        ...
        with st.spinner('Please wait...'):
            ...
            st.image(img_det, caption='Object segmentation results', width=800)
            st.divider()
            ...
            normalized_objs = ' '.join(objs.replace(',', ' ').split()).lower()
            persons= 'person' in normalized_objs.split()
            ...
            if persons:
                ...
                for result in results:
                    if len(result) > 0:
                        name_arr = result.identity.values
                        distance = result.distance.values
                        if name_arr is not None and len(name_arr) > 0:
                            name = name_arr[0]
                            distance = distance[0]
    
                            pic = Image.open(name)
                            st.image(pic, caption=f"Similarity: {distance:.2f}", width=150)
                            
                            found.append(
                                name.replace(f'{DB_PATH}/', '').replace('.jpg', '')
                            )                        
                ...
                ####################################
                st.write('##### Emotion recongntions results')
                results = DeepFace.analyze(np.array(img), enforce_detection = False)
                for i, res in enumerate(results):
                    emotion = res['dominant_emotion']
                st.write(f'The detected emotion is {emotion}')
                msg = '{} - file "{}" {}\n'.format(
                    datetime.datetime.now(),
                    file_name,
                    'emotion detected ' + emotion if emotion else 'no emotions found'
                )
                with open('history.log', 'a') as file:
                    file.write(msg)

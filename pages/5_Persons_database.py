...
st.sidebar.header('Faces database')
st.header('Database contains images of different people', divider='rainbow')

st.markdown(
    f"""
    Here you can see all images of the people 
    and update databes with new photos.
    """
)
st.divider()
...
def imgs_data(path):
    ...
    data = [{
        'img_name': f.replace(f'{path}/', '').replace('.jpeg', ''),
        'img_path': f'{path}/{f}'
    } for f in img_files]
    return data
